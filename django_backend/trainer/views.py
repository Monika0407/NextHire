...
def trainer_analytics_view(request):
    ...
    approved_students = TraineeProfile.objects.filter(approval_status='approved').count()
    pending_students = TraineeProfile.objects.filter(approval_status='pending').count()
    rejected_students = TraineeProfile.objects.filter(approval_status='rejected').count()
    active_jobs = Job.objects.filter(is_active=True).count()
    placed_students = TraineeProfile.objects.filter(current_status='Placed').count()
    placement_recommendations = CandidateRecommendation.objects.count()
    recruiter_count = RecruiterProfile.objects.count()
    course_breakdowns = Course.objects.annotate(student_count=Count('students')).order_by('-student_count')[:6]
    batch_breakdowns = TraineeProfile.objects.values('batch_code').annotate(batch_size=Count('id')).order_by('-batch_size')[:6]
    ...
    context = {
        ...
        'course_breakdowns': course_breakdowns,
