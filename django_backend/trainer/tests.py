...
class PlacementOfficerModuleTests(TestCase):
    ...
    def test_placement_officer_analytics_page(self):
        """Verify the trainer analytics page is accessible and shows key metrics."""
        self.client.login(email='trainer@nexthire.net', password='officerpassword')
        response = self.client.get(reverse('trainer:analytics'))
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, 'Placement Officer Intelligence')
