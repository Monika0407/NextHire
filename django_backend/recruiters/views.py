...
def recruiter_analytics_view(req):
    ...
    Provides hiring performance metrics, top job demand, and conversion ratios.
    """
    recruiter = get_object_or_404(RecruiterProfile, user=req.user)
    posted_jobs = recruiter.jobs.all().order_by('-posted_at')
    all_applications = Application.objects.filter(job__recruiter=recruiter)

    placed_count = all_applications.filter(status__in=[Status.OFFER_EXTENDED, Status.OFFER_ACCEPTED]).count()
    total_applicants = all_applications.count()

    applications_by_status = {
        'PENDING': all_applications.filter(status=Status.PENDING).count(),
        'SHORTLISTED': all_applications.filter(status=Status.SHORTLISTED).count(),
        'TESTING': all_applications.filter(status=Status.TESTING).count(),
        'INTERVIEWING': all_applications.filter(status=Status.INTERVIEWING).count(),
        'OFFERED': all_applications.filter(status__in=[Status.OFFER_EXTENDED, Status.OFFER_ACCEPTED]).count(),
        'REJECTED': all_applications.filter(status=Status.REJECTED).count(),
    }

    if total_applicants > 0:
        conversion_rate = int((placed_count / total_applicants) * 100)
    else:
        conversion_rate = 0

    top_jobs = posted_jobs.annotate(application_count=Count('applications')).order_by('-application_count')[:5]
    top_candidates = (
        all_applications
        .values('trainee__user__email')
