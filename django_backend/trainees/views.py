...
def trainee_analytics_view(req):
    ...
    applications_by_status = {
        'PENDING': applications.filter(status=Status.PENDING).count(),
        'SHORTLISTED': applications.filter(status=Status.SHORTLISTED).count(),
        'TESTING': applications.filter(status=Status.TESTING).count(),
        'INTERVIEWING': applications.filter(status=Status.INTERVIEWING).count(),
        'OFFERED': applications.filter(status__in=[Status.OFFER_EXTENDED, Status.OFFER_ACCEPTED]).count(),
        'REJECTED': applications.filter(status=Status.REJECTED).count(),
    }
    
    # Generate dynamic analytics trend metrics mapping over interview performance
    from interviews.models import Interview
    interviews = Interview.objects.filter(application__trainee=trainee, rating_score__isnull=False).order_by('scheduled_at')
    
    interview_trend_chart = []
    for i, interview in enumerate(interviews):
        interview_trend_chart.append({'label': f'Int {i+1}', 'score': float(interview.rating_score)})
        
    if not interview_trend_chart:
         interview_trend_chart = [{'label': 'N/A', 'score': 0.0}]
         
    num_points = len(interview_trend_chart)
    if num_points == 1:
        interview_trend_chart[0]['x'] = 200
        interview_trend_chart[0]['y'] = 100 - (interview_trend_chart[0]['score'] / 5.0 * 80) - 10
        path_d = ""
    else:
        x_step = 380 / (num_points - 1)
        path_segments = []
        for i, item in enumerate(interview_trend_chart):
            item['x'] = 10 + i * x_step
            item['y'] = 100 - (item['score'] / 5.0 * 80) - 10
            if i == 0:
                path_segments.append(f"M {item['x']} {item['y']}")
            else:
                path_segments.append(f"L {item['x']} {item['y']}")
        path_d = " ".join(path_segments)

    total_apps = len(applications)
    if total_apps > 0:
        p1 = (applications_by_status['PENDING'] / total_apps) * 100
        p2 = p1 + (applications_by_status['SHORTLISTED'] / total_apps) * 100
        p3 = p2 + (applications_by_status['INTERVIEWING'] / total_apps) * 100
        p4 = p3 + (applications_by_status['OFFERED'] / total_apps) * 100
        
        # Colors: blue-500, cyan-500, amber-500, emerald-500, red-500
        conic_gradient = f"conic-gradient(#3b82f6 0% {p1}%, #06b6d4 {p1}% {p2}%, #f59e0b {p2}% {p3}%, #10b981 {p3}% {p4}%, #ef4444 {p4}% 100%)"
    else:
        conic_gradient = "conic-gradient(#1e293b 0% 100%)"
    ...
    context = {
        ...
        'gpa_gap': gpa_gap,
        'app_status_counts': applications_by_status,
        'interview_trend': interview_trend_chart,
        'path_d': path_d,
